...
class clientes extends PublicController
{
    ...
    private function init()
    {
        ...
        if (!isset($this->_modeStrings[$this->_viewData["mode"]])) {
            ...
            \Utilities\Site::redirectToWithMsg("index.php?page=sedah.clientes.clientesLista", "Sucedio un error al procesar la pagina");
        }
    ...
    private function handlePost()
    {
        ...
        if (!isset($_SESSION["clientes_crsxToken"]) || $_SESSION["clientes_crsxToken"] !== $this->_viewData["crsxToken"]) {
            ...
            \Utilities\Site::redirectToWithMsg("index.php?page=sedah.clientes.clientesLista", "Ocurrio un error, no se puede procesar");
        }
        ...
        if (isset($this->_viewData["errors"]) && count($this->_viewData["errors"]) > 0) {
            ...
            switch ($this->_viewData["mode"]) {
                case "INS":
                    $result = \Dao\Sedah\clientes::insertarclientes(
                    ...
                    if ($result) {
                        \Utilities\Site::redirectToWithMsg(
                            "index.php?page=sedah.clientes.clientesLista",
                            "¡Registro guardado exitosamente!"
                        );
                    }
                    break;
                case "UPD":
                    $result = \Dao\Sedah\clientes::actualizarclientes(
                    ...
                    break;
                case "DEL":
                    $result = \Dao\Sedah\clientes::eliminarclientes(
                        $this->_viewData["idCliente"]
                    );
                    if ($result) {
                        \Utilities\Site::redirectToWithMsg(
    ...
    private function prepareViewData()
    {
        if ($this->_viewData["mode"] == "INS") {
            ...
            $tmpclientes = \Dao\Sedah\clientes::obtenerPorId(intval($this->_viewData["idCliente"], 10));
    ...
    public function run(): void
    {
        ...
        Renderer::render("sedah/clientes", $this->_viewData);
    }
}
